...
class Worker:
    ...
    def _process_audio(self, key_press_at: str, segment_queue: queue.Queue) -> None:
        """STT 线程：消费音频片段，生成原始转录文本并送入文本队列"""
        try:
            ...
            while True:
                ...
                if isinstance(item, tuple) and item[0] is _SENTINEL:
                    ...
                    # 将哨兵值传递给文本队列
                    self._text_queue.put((_SENTINEL, key_release_at))
                    break
            ...
            # 发生严重错误时，也要通知文本线程结束，防止其永久阻塞
            self._text_queue.put((_SENTINEL, datetime.now().strftime(self._TIME_FMT)))

    def _process_text(self, key_press_at: str, text_queue: queue.Queue) -> None:
        """LLM 线程：消费 STT 结果，进行精修、注入文本并记录历史"""
        try:
