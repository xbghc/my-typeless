...
class Worker:
    ...
    采用流水线架构：
    Thread 1 (Audio): 录音片段 -> STT -> 文本队列
    Thread 2 (Text):  文本队列 -> LLM 精修 -> 最终结果
    ...
    def stop_recording_and_process(self) -> None:
        ...
        # 发送哨兵值（附带按键释放时间），告知 Audio 线程不再有新片段
    ...
    def _process_text(self, text_queue: queue.Queue, key_press_at: str) -> None:
        ...
        try:
            ...
            # 拼接全部结果
            full_raw_text = "".join(raw_text_parts)
            ...
            self.events.emit("result_ready", full_refined_text)
