...
class Worker(QObject):
    ...
    def _llm_task(self, key_press_at: str, text_queue: queue.Queue) -> None:
        ...
        try:
            ...
            # 拼接全部结果
            raw_text_full = "".join(raw_parts)
            refined_text_full = "".join(refined_parts)
            logger.debug("Full STT result: %r", raw_text_full)
            logger.debug("Full LLM result: %r", refined_text_full)
            ...
            inject_text(refined_text_full)
            logger.debug("Text injected successfully")

            # 记录历史
            add_history(
                raw_text_full, refined_text_full,
            ...
            self.result_ready.emit(refined_text_full)
