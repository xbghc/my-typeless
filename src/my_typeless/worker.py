...
class Worker:
    ...
    def _process_audio(self, key_press_at: str, segment_queue: queue.Queue, text_queue: queue.Queue) -> None:
        """音频处理消费线程：逐段 STT → 放入 text_queue"""
        ...
        try:
            ...
            # 放入哨兵值，通知 text_queue 结束
            if key_release_at is None:
                key_release_at = datetime.now().strftime(self._TIME_FMT)
            text_queue.put((_SENTINEL, raw_text, key_release_at, stt_done_at))

        except Exception as e:
            self._handle_error(e)
            # 在异常情况下也要通知下游结束，避免 text 线程死锁
            if key_release_at is None:
                key_release_at = datetime.now().strftime(self._TIME_FMT)
            if stt_done_at is None:
                stt_done_at = datetime.now().strftime(self._TIME_FMT)
            raw_text = "".join(transcription_parts)
            text_queue.put((_SENTINEL, raw_text, key_release_at, stt_done_at))

        # 注意：这里不再 emit state_changed idle，因为整个流程还没结束，由 process_text 负责

    def _process_text(self, key_press_at: str, text_queue: queue.Queue) -> None:
        """文本处理消费线程：LLM 精修 → 拼接 → 注入文本"""
        try:
            ...
            while True:
                ...
                if isinstance(item, tuple) and item[0] is _SENTINEL:
                    raw_text, key_release_at, stt_done_at = item[1], item[2], item[3]
                    break

                # item is text segment from STT
                text = item
            ...
            if not refined_text.strip():
                logger.debug("Empty LLM result, skipping")
                return
